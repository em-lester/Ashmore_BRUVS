...
m.dir <- paste(w.dir,"Model_Outputs/Likelihood fed", sep="/")
...
name<-"Ashmore_BRUVS_likelihood_fed"
dir()
...
ggComplexity <- ggplot(dat, aes(x=Complexity, y=response,fill=Size_class, colour=Size_class))+ facet_wrap(~Size_class)+
  geom_violin(size=3, alpha=0.7)+
  scale_fill_viridis_d()+
  scale_colour_viridis_d()+
  theme_classic()
ggComplexity+ theme(axis.text.x=element_text(size=15),
                       axis.text.y=element_text(size=15),
                       axis.title=element_text(size=20),
                       strip.text =element_text(size=12),
                       legend.position="none")
